...
- (void)viewDidLoad {
    ...
    self.languageL.text = NSLocalizedString(@"语言设置", nil);
    self.parkingModeL.text = NSLocalizedString(@"停车监控", nil);
    ...
    [self.languageSegment setTitle:NSLocalizedString(@"英语", nil) forSegmentAtIndex:0];
    [self.languageSegment setTitle:NSLocalizedString(@"繁体中文", nil) forSegmentAtIndex:1];
...
- (void)updateUIWithSettingInfo{
    ...
    if (settingInfo) {
        ...
        self.languageSegment.selectedSegmentIndex = settingInfo.language;

        self.EdogSwitch.on = settingInfo.edog;
        self.parkingModeSwitch.on = settingInfo.parkingMode;
...
- (IBAction)toggleOarkingModeSwitch:(UISwitch *)sender {
    
    if (!self.isSettable) return;
      
      MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:self.navigationController.view animated:YES];
      
      NSString *value = [APKDVR sharedInstance].settingInfo.parkingModeMap[sender.isOn];
      NSString *property = @"Camera.Menu.PowerOnGsensorSensitivity";
      __weak typeof(self)weakSelf = self;
      [[APKDVRCommandFactory setCommandWithProperty:property value:value] execute:^(id responseObject) {
          
          [APKDVR sharedInstance].settingInfo.parkingMode = sender.isOn;
          [hud hide:YES];
          [APKAlertTool showAlertInView:self.view andText:NSLocalizedString(@"设置成功", nil)];
...
- (IBAction)updateLanguageSegment:(UISegmentedControl *)sender {
    
    if (!self.isSettable) return;
        
        MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:self.navigationController.view animated:YES];
        
        NSString *value = [APKDVR sharedInstance].settingInfo.languageMap[sender.selectedSegmentIndex];
        NSString *property = @"Camera.Menu.ApkLanguage";
        __weak typeof(self)weakSelf = self;
        [[APKDVRCommandFactory setCommandWithProperty:property value:value] execute:^(id responseObject) {
            
            [APKDVR sharedInstance].settingInfo.language = sender.selectedSegmentIndex;
            [hud hide:YES];
            ...
            [APKAlertTool showAlertInViewController:weakSelf title:nil message:NSLocalizedString(@"设置失败！", nil) confirmHandler:^(UIAlertAction *action) {
                ...
                sender.selectedSegmentIndex = [APKDVR sharedInstance].settingInfo.language;
            }];
        }];
}
...
- (NSArray *)rowHeights{
    ...
    if (!_rowHeights) {
        _rowHeights = @[@(62),@(62),@(62),@(98),@(98),@(98),@(98),@(98),@(62),@(62),@(62),@(62),@(62),@(62),@(62),@(62),@(62)];
    }
    return _rowHeights;
}
...
//        _cells = @[self.recordSoundCell,self.motionDetectionCell,self.clipDurationCell,self.gSensorCell,self.LCDLightCell,self.watermarkCell1,self.dateFormatCell,self.exposureCell,self.correctCameraClockCell,self.formatCell,self.networkConfigureCell,self.factoryResetCell,self.helpCell,self.aboatCell,self.DBValueCell];
        _cells = @[self.EdogCell,self.recordSoundCell,self.parkingModeCell,self.languageCell,self.clipDurationCell,self.volumeCell,self.gSensorCell,self.exposureCell,self.correctCameraClockCell,self.formatCell,self.networkConfigureCell,self.factoryResetCell,self.helpCell,self.aboatCell];
    }
    return _cells;
}
